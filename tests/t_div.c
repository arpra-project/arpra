...
int main (int argc, char *argv[])
{
    ...
    for (i = 0; i < test_n; i++) {
        ...
        // 1) MPFA x contains 0, MPFA y contains 0 and MPFA z = NaN.
        // 2) MPFA y contains 0 and MPFA z = Inf.
        // 3) MPFA z contains MPFI z.
        // 4) MPFA z unbounded and MPFI z unbounded.
        test_bivariate(mpfa_div, mpfi_div);
        if (mpfa_has_zero_p(x_A) && mpfa_has_zero_p(y_A) && mpfa_nan_p(z_A)) {
            test_log_printf("Result (unshared symbols): PASS\n\n");
        }
        else if (mpfa_has_zero_p(y_A) && mpfa_inf_p(z_A)) {
            test_log_printf("Result (unshared symbols): PASS\n\n");
        }
