...
int main (int argc, char *argv[])
{
    ...
    shared_log = fopen("mul_shared.log", "w");
    partshared_log = fopen("mul_partshared.log", "w");
    unshared_log = fopen("mul_unshared.log", "w");
    ...
    for (i = 0; i < test_n; i++) {
        ...
        // 1) Arpra x1 contains 0, Arpra x2 = Inf and Arpra y = NaN.
        // 2) Arpra x1 = Inf, Arpra x2 contains 0 and Arpra y = NaN.
        // 3) Arpra y contains MPFI y.
        test_bivariate(arpra_mul, mpfi_mul);
        if (arpra_has_zero_p(&x1_A) && arpra_inf_p(&x2_A) && arpra_nan_p(&y_A)) {
            test_log_printf("Result (unshared symbols): PASS\n\n");
        }
        else if (arpra_inf_p(&x1_A) && arpra_has_zero_p(&x2_A) && arpra_nan_p(&y_A)) {
            test_log_printf("Result (unshared symbols): PASS\n\n");
        }
