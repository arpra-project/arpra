...
int main (int argc, char *argv[])
{
    ...
    shared_log = fopen("sub_shared.log", "w");
    partshared_log = fopen("sub_partshared.log", "w");
    unshared_log = fopen("sub_unshared.log", "w");
    ...
    for (i = 0; i < test_n; i++) {
        ...
        // 2) Arpra y unbounded and MPFI y unbounded.
        ...
        test_bivariate(arpra_sub, mpfi_sub);
        if (arpra_bounded_p(&y_A) == mpfi_bounded_p(y_I)) {
