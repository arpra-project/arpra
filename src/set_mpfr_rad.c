...
void arpra_set_mpfr_rad (arpra_range *z, const arpra_mpfr *centre, const arpra_mpfr *radius)
{
    ...
    if (mpfr_nan_p(centre) || mpfr_nan_p(radius) || (mpfr_cmp_ui(radius, 0) < 0)) {
        arpra_set_nan(z);
        return;
    }
    if (mpfr_inf_p(centre) || mpfr_inf_p(radius)) {
        arpra_set_inf(z);
        return;
    }
    ...
    // MPFI set_fr
    mpfi_set_fr(&(z->true_range), centre);
    mpfr_sub(&(z->true_range.left), &(z->true_range.left), radius, MPFR_RNDD);
    mpfr_add(&(z->true_range.right), &(z->true_range.right), radius, MPFR_RNDU);
