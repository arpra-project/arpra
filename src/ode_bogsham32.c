...
typedef struct bogsham32_scratch_struct
{
    ...
    arpra_range ah[bogsham32_stages][bogsham32_stages];
...
static void bogsham32_init (arpra_ode_stepper *stepper, arpra_ode_system *system)
{
    ...
    for (x_i = 0; x_i < system->dims; x_i++) {
        ...
        arpra_init2(&(scratch->x_new_3[x_i]), prec_x);
        arpra_init2(&(scratch->x_new_2[x_i]), prec_x);
    }
    for (k_i = 0; k_i < bogsham32_stages; k_i++) {
        for (k_j = 0; k_j < bogsham32_stages; k_j++) {
...
static void bogsham32_clear (arpra_ode_stepper *stepper)
{
    ...
    for (x_i = 0; x_i < system->dims; x_i++) {
        ...
        arpra_clear(&(scratch->x_new_3[x_i]));
        arpra_clear(&(scratch->x_new_2[x_i]));
    }
    for (k_i = 0; k_i < bogsham32_stages; k_i++) {
        for (k_j = 0; k_j < bogsham32_stages; k_j++) {
...
static void bogsham32_step (arpra_ode_stepper *stepper, const arpra_range *h)
{
    ...
    for (x_i = 0; x_i < system->dims; x_i++) {
        ...
        arpra_set_precision(&(scratch->x_new_3[x_i]), prec_x);
        arpra_set_precision(&(scratch->x_new_2[x_i]), prec_x);
    }
    for (k_i = 0; k_i < bogsham32_stages; k_i++) {
        for (k_j = 0; k_j < bogsham32_stages; k_j++) {
    ...
    for (x_i = 0; x_i < system->dims; x_i++) {
        ...
        for (k_i = 0; k_i < bogsham32_stages; k_i++) {
            for (k_j = 0; k_j < k_i; k_j++) {
                x_new = (k_j == 0) ? system->x : scratch->x_new_3;
                arpra_mul(&(scratch->temp_x), &(scratch->ah[k_i][k_j]), &(scratch->k[k_j][x_i]));
                arpra_add(&(scratch->x_new_3[x_i]), &(x_new[x_i]), &(scratch->temp_x));
            }
            system->f(scratch->k[k_i],
                      &(scratch->temp_t[k_i]), scratch->x_new_3,
                      x_i, system->params);
        }

        // Compute second-order approximation.
        for (k_j = 0; k_j < bogsham32_stages; k_j++) {
            x_new = (k_j == 0) ? system->x : scratch->x_new_2;
            arpra_mul(&(scratch->temp_x), &(scratch->bh_2[k_j]), &(scratch->k[k_j][x_i]));
            arpra_add(&(scratch->x_new_2[x_i]), &(x_new[x_i]), &(scratch->temp_x));
        }
    }

    // Advance system.
    arpra_add(system->t, system->t, h);
    x_new = scratch->x_new_3;
    scratch->x_new_3 = system->x;
    system->x = x_new;
}
