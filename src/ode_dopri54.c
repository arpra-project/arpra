...
static void dopri54_init (arpra_ode_stepper *stepper, arpra_ode_system *system)
{
    ...
    for (i = 0; i < system->dims; i++) {
        ...
        arpra_init2(&(scratch->x_new_5[i]), prec);
        arpra_init2(&(scratch->x_new_4[i]), prec);
    }
...
static void dopri54_step (arpra_ode_stepper *stepper, const arpra_range *h)
{
    ...
    for (i = 0; i < system->dims; i++) {
        ...
        arpra_set_precision(&(scratch->x_new_5[i]), prec);
        arpra_set_precision(&(scratch->x_new_4[i]), prec);
    }
    ...
    for (i = 0; i < system->dims; i++) {
        arpra_mul(&(scratch->x_new_5[i]), &(scratch->k_weights_5[0]), &(scratch->k[0][i]));
        arpra_add(&(scratch->x_new_5[i]), &(system->x[i]), &(scratch->x_new_5[i]));
    }
    arpra_add(&(scratch->temp), system->t, &(scratch->k_weights_5[0]));
    ...
    for (i = 0; i < system->dims; i++) {
        arpra_mul(&(scratch->x_new_5[i]), &(scratch->k_weights_5[0]), &(scratch->k[1][i]));
        arpra_add(&(scratch->x_new_5[i]), &(system->x[i]), &(scratch->x_new_5[i]));
    }
    arpra_add(&(scratch->temp), system->t, &(scratch->k_weights_5[0]));
