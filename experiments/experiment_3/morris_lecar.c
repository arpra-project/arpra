...
void file_write (const arpra_range *A, unsigned long grp_size, FILE **f)
{
    unsigned long i;

    for (i = 0; i < grp_size; i++) {
        mpfr_out_str(f[i], 10, 40, &(A[i].true_range.left), MPFR_RNDN);
        fputs(" ", f[i]);
        mpfr_out_str(f[i], 10, 40, &(A[i].true_range.right), MPFR_RNDN);
        fputs("\n", f[i]);
    }
}
...
int main (int argc, char *argv[])
{
    ...
    // Input and conductance files
    FILE *in_file = fopen("input.dat", "w");
    FILE *g_file = fopen("conductance.dat", "w");

    enum grps { grp_nrn1_N, grp_nrn1_V, grp_syn_exc_R, grp_syn_exc_S };
    ...
    arpra_range GL, VL, GCa, VCa, GK, VK, V1, V2, V3, V4, phi, C,
                ...
                temp1, temp2, M_ss, N_ss, in1_V_lo, in1_V_hi;
    ...
    for (i = 0; i < p_syn_exc_size; i++) {
        mpfr_out_str(g_file, 10, 80, &(syn_exc_GSyn[i].centre), MPFR_RNDN);
        fprintf(g_file, "\n");
    }
    ...
    arpra_uint sys_dims[4] = {
        p_nrn1_size, p_nrn1_size, p_syn_exc_size, p_syn_exc_size
    };
    arpra_ode_f sys_f[4] = {
        dNdt, dVdt, dRdt, dSdt
    };
    void *sys_params[4] = {
        &params_nrn1_N, &params_nrn1_V, &params_syn_exc_R, &params_syn_exc_S
    };
    arpra_range *sys_x[4] = {
        nrn1_N, nrn1_V, syn_exc_R, syn_exc_S
    };
